import { url as e, beurl as t } from "../utility/js/constants.js"; let headLeft = document.getElementById("head-left"); headLeft.innerHTML = `
...
const overlay = document.getElementById("iframeOverlay");
  ...
  // Activate game (disable scroll)
  overlay.addEventListener("click", () => {
    overlay.classList.add("hidden");
    document.body.classList.add("no-scroll");
  });

  // Click outside game → restore scroll
  document.addEventListener("click", (e) => {
    if (!gameBox.contains(e.target)) {
...
const gameHolder2 = document.getElementById("gameHolder2");
  ...
  game_data.forEach(game => {
    ...
    anchor.appendChild(img);
    wrapper.appendChild(anchor);
    gameHolder2.appendChild(wrapper);
  });
}
